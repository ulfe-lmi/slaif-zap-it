...
| BLIP3 verification | Pinned FP16 holder; each QA call reuses one deterministic mask-isolated pair with exact selected-pixel preservation, exterior dimming and configured contour; residency remains capacity-selected | Public L2/L3 fields when executed; L3 debug returns the exact paired PNG under a fixed numeric name; this is not an accuracy guarantee |
