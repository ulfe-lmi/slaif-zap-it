...
| Candidate views for CLIP/BLIP3 | CLIP uses an untouched rectangular raw source crop; trusted CLI may explicitly retain its exact-mask compatibility view; BLIP3 defaults to the existing Euclidean single-image composition, with an explicit `centroid_radial_mask_chord` fallback only after its exact containment rejection | Effective policy at every response level; L3 composition records and exact lossless processor/QA PNGs; fallback records include deterministic centroid/contour/radial adjustment evidence; this is pixel-boundary evidence, not an accuracy guarantee |
